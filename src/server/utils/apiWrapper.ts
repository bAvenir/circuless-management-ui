...
export async function apiWrapper<T>(event: H3Event<EventHandlerRequest>, request: (data: ApiData) => Promise<T>, options?: Options): Promise<T> {
  try {
    const params = getRouterParams(event)
    const query = getQuery(event)
    const body = event.method === 'GET' || event.method === 'HEAD' ? undefined : await readBody(event)
    ...
    const requestData: ApiData = { params, query, body, user: event.context.user }
...
function validateRequest(
  ...
  if (params && Object.keys(params).length > 0 && !schemas?.params) {
    throw new ApplicationError('Params are not allowed without validation schema', HttpStatusCode.INTERNAL_SERVER_ERROR)
  }

  if (query && Object.keys(query).length > 0 && !schemas?.query) {
    throw new ApplicationError('Query is not allowed without validation schema', HttpStatusCode.INTERNAL_SERVER_ERROR)
  }

  if (body && Object.keys(body).length > 0 && !schemas?.body) {
    throw new ApplicationError('Body is not allowed without validation schema', HttpStatusCode.INTERNAL_SERVER_ERROR)
  }
...
function validateSchema(schema?: Joi.Schema, data?: any, type?: string) {
  if (schema) {
    ...
    if (res.error) {
      throw new ValidationError(`Validation error in ${type}`, HttpStatusCode.BAD_REQUEST, res.error.details)
    }
  }
}
