...
import type { InviteRepresentation, MemberRepresentation, OrganisationRepresentation } from './keycloak'
...
class UserManager {
  invite = async (event: H3Event<EventHandlerRequest>, data: userTypes.InviteBody) => {
  ...
  syncWithKc = async (event: H3Event<EventHandlerRequest>) => {
    ...
    for (const realm of realms) {
      const kcOrganisatoins = await keycloak.getOrganisations(event, realm)
      ...
      for (const kcOrganisation of kcOrganisatoins) {
        const members = await keycloak.getOrganisationMembers(event, kcOrganisation.id!, realm)
      ...
      for (const kcUsers of kcUsersWithOrganisation) {
        const usersAffected: Affected = { created: [], updated: [], deleted: [] }

        for (const kcUser of kcUsers.users) {
          const { user, status } = await _updateUserToMatchKcUser(realm, kcUser, kcUsers.organisation)
          if (status === SyncStatus.CREATED) usersAffected.created.push(user)
          if (status === SyncStatus.UPDATED) usersAffected.updated.push(user)
        }

        affected.created.push(...usersAffected.created)
        affected.updated.push(...usersAffected.updated)
      }
      affected.deleted.push(...(await _deleteUsersNotInKcUsers(realm, allKcUsers)))
    }
    return affected
  }

  syncMyUserWithKc = async (event: H3Event<EventHandlerRequest>, realm: Realm) => {
    const { sub, email, given_name: givenName, family_name: familyName, kcOrganisation } = keycloak.validateClaims(event)
...
async function _deleteUsersNotInKcUsers(realm: Realm, kcUsers: MemberRepresentation[]) {
