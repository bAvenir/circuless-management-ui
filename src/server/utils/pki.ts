...
class PKI {
  ...
  async init() {
    ...
    if (!this.pkiConfig.url || !this.pkiConfig.user || !this.pkiConfig.password) {
      console.error('PKI configuration is incomplete. Please check NUXT_PKI_* environment variables.')
      throw new ApplicationError('PKI service configuration is missing', HttpStatusCode.INTERNAL_SERVER_ERROR)
    }
  ...
  public async signCertificateRequest(options: CertificateSigningOptions): Promise<SignedCertificateResult> {
    ...
    const cert = await pkiApiWrapper(async () => {
      ...
      const auth = Buffer.from(`${this.pkiConfig.user}:${this.pkiConfig.password}`).toString('base64')

      return await $fetch<MyCertType>(`${endpoint}/pki/certificates/${certId}`, {
        method: 'POST',
        headers: {
  ...
  public extractCertificateInfo(certificatePem: string): CertificateInfo | null {
    try {
      ...
      cert.subject.attributes.forEach((attr) => {
        switch (attr.name || attr.shortName || attr.type) {
          case 'commonName':
            clientName = typeof attr.value === 'string' ? attr.value : ''
            break
          case 'organizationName':
            // Use organization name directly as organisation ID
            organisationId = typeof attr.value === 'string' ? attr.value : ''
            break
  ...
  public async renewCertificate(certId: string): Promise<MyCertType> {
    return await pkiApiWrapper(async () => {
      ...
      return await $fetch<MyCertType>(`${endpoint}/pki/certificates/renew/${certId}`, {
        method: 'POST',
        headers: {
          Authorization: `Basic ${auth}`,
        },
  ...
  public async getRevocationList(): Promise<string> {
    return await pkiApiWrapper(async () => {
      ...
      return await $fetch<string>(`${endpoint}/pki/certificates/revocation-list`, {
        method: 'GET',
        headers: {
          Authorization: `Basic ${auth}`,
        },
  ...
  try {
    ...
    if (error instanceof FetchError) {
      ...
      if (error.response?._data?.detail && Array.isArray(error.response._data.detail)) {
        const validationErrors = error.response._data.detail as PkiValidationError[]
      ...
      throw new PKIError(errorMessage, error.response?.status || HttpStatusCode.INTERNAL_SERVER_ERROR, error.response?._data)
    }
    throw error
  }
}
