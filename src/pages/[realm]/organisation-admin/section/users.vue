<template>
    <div v-if="my" class="w-full h-full">
        ...
        <Dialog
            ...
            modal
            class="min-w-[375px] lg:min-w-[556px]"
            :draggable="false"
            :resizable="false"
        >
            <template #header>
...
const onUserInvited = async (data: userTypes.InviteBodyRealm) => {
    try {
        await realmUserStore.inviteUserToMyOrganisation(realm.value, data)
...
const onUserRemoved = async (userId: string) => {
    try {
        await realmUserStore.removeUsersFromMyOrganisation({
            userIds: [userId],
        })
